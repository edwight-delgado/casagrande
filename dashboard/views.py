...
class DashboardView(View):
    ...
    def get(self, *args, **kwargs):
        ...
        cursor.execute('SELECT core_payment.id, methods, SUM(amount) AS Total from core_order join core_payment ON core_order.payment_id=core_payment.id WHERE core_order.received=1 GROUP BY methods')
        payments = self.dictfetchall(cursor)
        cursor.close()
        cursor = connection.cursor()
        #payments = Payment.objects.raw('SELECT id, methods, SUM(amount) AS Total from core_payment GROUP BY methods')
        cursor.execute('SELECT SUM(amount) AS Total from core_order join core_payment ON core_order.payment_id=core_payment.id WHERE core_order.received==1  GROUP BY methods')
...
class DashboardOrderDetailsView(View):
    def get(self, *args, **kwargs):
        ...
        cursor = connection.cursor()
        #payments = Payment.objects.raw('SELECT id, methods, SUM(amount) AS Total from core_payment GROUP BY methods')
        cursor.execute(f'SELECT SUM(amount) AS Total from core_order join core_payment ON core_order.payment_id=core_payment.id WHERE core_order.user_id==1 AND core_order.received==1 GROUP BY methods')
        payment_total = cursor.fetchall()
